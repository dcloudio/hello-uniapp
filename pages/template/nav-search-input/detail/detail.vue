<template>
	<view class="wrapper">
		<view v-if="isHistory" class="history-box">
			...
			<view v-else class="no-data">You do not have a history yet</view>
		</view>
		<view v-else class="history-box">
			...
			<view v-else class="no-data">No related content searched</view>
		</view>
	</view>
</template>
	...
	export default {
		...
		methods: {
			...
			clearSearch() {
				uni.showModal({
					title: 'Tips',
					content: 'Do you clear the entire search history? This operation is not reversible.',
					success: res => {
						if (res.confirm) {
		...
		onNavigationBarSearchInputConfirmed(e) {
			...
			if (!text) {
				...
				uni.showModal({
					title: 'Tips',
					content: `You have entered the following"${text}",If you click OK, the search will be recorded in the history and returned. If you cancel without doing anything`,
					success: res => {
						if (res.confirm) {
		...
		onNavigationBarButtonTap() {
			uni.showModal({
				title: 'Tips',
				content: 'Click OK to change the content of the input box to abc',
				success: res => {
					if (res.confirm) {
